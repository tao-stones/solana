    ...
    D: TransactionData,
{
    ...
    let transaction_config_source =
        if let Some(transaction_config_view) = transaction.transaction_config() {
            // NOTE only txv1 has `transaction_config_view`, which must be validated for sanitized
            // transaction view.
        ...
        let message = match self.version() {
            ...
            TransactionVersion::V1 => {
                ...
                let config = solana_message::v1::TransactionConfig {
                    priority_fee: Some(config_view.priority_fee_lamports()),
                    compute_unit_limit: Some(config_view.compute_unit_limit()),
                    loaded_accounts_data_size_limit: Some(
                        config_view.loaded_accounts_data_size_limit(),
                    ),
                    heap_size: Some(config_view.requested_heap_size()),
                };
