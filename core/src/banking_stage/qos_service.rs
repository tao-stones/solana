...
impl QosService {
    ...
    fn select_transactions_per_cost<'a, Tx: TransactionWithMeta>(
        ...
        let remove_simple_vote_from_cost_model = bank
            .feature_set
            .is_active(&agave_feature_set::remove_simple_vote_from_cost_model::ID);
        ...
        let select_results = transactions
            ...
            .map(|(tx, cost)| match cost {
                Ok(cost) => match cost_tracker.try_add(&cost, remove_simple_vote_from_cost_model) {
                    Ok(UpdatedCosts {
                        updated_block_cost,
